...
for cr = 1:length(options.cRates)
    ...
    for p = 1:2
        ...
        for w = 1:length(options.wtSi)
            ...
            plot(res.SoC*100, real(res.(phases{p}).sigma_t_surface) /... 
                1e6, 'Color', cmap_mech(w,:), 'LineWidth', 2, ...
                'DisplayName', sprintf('%.0f%% Si: \\sigma_t',...
                options.wtSi(w)*100));
            
            % Plot Radial Center Stress (Dashed line)
            plot(res.SoC*100, real(res.(phases{p}).sigma_r_center) /...
                1e6, 'Color', cmap_mech(w,:), 'LineWidth', 1.5,...
        ...
        if p == 1
            legend('Location', 'southwest', 'NumColumns',...
                1, 'FontSize', 8);
            % Dynamic placement of helper text based on Y-axis
            y_limits = ylim;

            text(5, y_limits(1) + 0.15*diff(y_limits),...
                'Solid: Tangential (Surface)', 'FontSize',...
                9, 'FontAngle', 'italic');
            text(5, y_limits(1) + 0.08*diff(y_limits),...
                'Dash: Radial (Center)', 'FontSize', 9,...
                'FontAngle', 'italic');
        end
    end
    sgtitle(sprintf(...
        'Intra-Particle Stress Comparison: %.1fC Discharge',...
        options.cRates(cr)), ...
            'FontSize', 14, 'FontWeight', 'bold');
end
...
%% ═══════════════════════════════════════════════════════════════════════
figure('Name', 'Total Pack Stack Stress', 'Color', 'w',...
...
% Legend for C-Rates
y_max = max(ylim);
text(2, y_max*0.95, 'Line Styles (C-Rate):', 'FontWeight',...
    'bold', 'FontSize', 10);

for cr = 1:length(options.cRates)
    text(5, y_max*(0.95 - cr*0.06), sprintf('%s  %.1f C',...
        line_styles{mod(cr-1,3)+1}, options.cRates(cr)), 'FontSize', 10);
end
