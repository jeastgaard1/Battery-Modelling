...
if options.bool.ini==1  
    if battery_res.P(1,1)<0                                                 %Discharge
        battery_res.OCV(1,1)=interp1(param.fit.OCV.DCH.SoC,param.fit.OCV.DCH.U,battery_res.SoC(1,1));
    else                                                                   %Charge
        battery_res.OCV(1,1)=interp1(param.fit.OCV.CH.SoC,param.fit.OCV.CH.U,battery_res.SoC(1,1));
    end
else
    if battery_res.P(1,1)<0                                           %Discharge
        battery_res.OCV(2,1)=interp1(param.fit.OCV.DCH.SoC,param.fit.OCV.DCH.U,battery_res.SoC(2,1));
    else                                                                   %Charge
        battery_res.OCV(2,1)=interp1(param.fit.OCV.CH.SoC,param.fit.OCV.CH.U,battery_res.SoC(2,1));
    end
end
