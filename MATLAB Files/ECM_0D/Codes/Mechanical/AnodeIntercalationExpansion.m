...
x = param.StochiometricNumbers.Anode_x0 ...
    + (param.StochiometricNumbers.Anode_x100 - param.StochiometricNumbers.Anode_x0) * SoC(1,:,:,:,:);
...
u_Rp_anode= 1/3 * param.ParticleDiffusion.Anode.R * dV_crystal_anode *0.01;

% Berechnung der Volumenänderung "dV_p_anode" des repräsentativen runden Partikels:
dV_p_anode= ((4/3* pi * (param.ParticleDiffusion.Anode.R + u_Rp_anode).^3) ...
    - (4/3* pi * (param.ParticleDiffusion.Anode.R).^3)) / (4/3* pi * (param.ParticleDiffusion.Anode.R).^3);

% Berechnung der Dickenänderung "dL_int_anode" der Dickenelemente der Anode:
dL_int_anode = param.ActivematerialVolumeFraction.Anode*dV_p_anode ...
    *param.Thickness.Anode/options.seg_thickness; 
